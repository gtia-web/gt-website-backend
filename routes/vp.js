...
router.get('/', authentication.checkAuthenticated, async (req, res) => {
  ...
  const pointRequest = await PointsRequest.find(searchParams);
  //   console.log('wooh' + pointRequest);

  //   pointsReciepts = await PointsReceipt.findById(req.user._id);
  //   console.log(pointsReciepts);
  //   pointsRequest = await PointsRequest.findById(req.user._id);
  //   console.log(pointsRequest);
  //   console.log('hey');

  res.render('vp.ejs', { user: userData });
});
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const pointRequest = await PointsRequest.findByIdAndUpdate(
        req.query.id,
        updatePoint,
        { new: true }
      );
