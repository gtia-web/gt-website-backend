...
async function getEmailTransporter() {
  const myAccessToken = await getClient().getAccessToken();
...
async function sendMain(mailOptions) {
  await getEmailTransporter().sendMail(mailOptions, function (error, info) {
    if (error) {
      console.log(error);
    } else {
...
async function saveImageToDrive(image_data) {
  ...
  var media = {
    ...
    body: fs.createReadStream(
      "public/" + image_data.Path + "/" + image_data.Filename
    ), //files/photo.jpg')
  };
